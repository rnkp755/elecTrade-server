...
const getAllTrades = asyncHandler(async (req, res, next) => {
      ...
      const [trades, totalTrades] = await Promise.all([
            Trade.aggregatePaginate({}, options),
...
const getMyTrades = asyncHandler(async (req, res, next) => {
      ...
      const [trades, totalTrades] = await Promise.all([
            Trade.aggregatePaginate({ createdBy: userId }, options),
            Trade.countDocuments({ createdBy: userId }),
      ]);

      if (!trades) throw new APIError(404, 'No trades found')
